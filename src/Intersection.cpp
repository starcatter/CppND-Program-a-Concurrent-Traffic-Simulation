...
bool Intersection::debugOut = true;
...
void Intersection::addVehicleToQueue( std::shared_ptr<Vehicle> vehicle )
{
	...
	vehicle->setState(VehicleState::waitingForIntersection);
	...
	if ( _trafficLight.getCurrentPhase() != TrafficLightPhase::green )
	{
		...
		{
			lck.lock();
			std::cout << "Intersection #" << _id << ": Vehicle #" << vehicle->getID() << " is waiting for a green light..." << std::endl;
			lck.unlock();
		}
		
		vehicle->setState(VehicleState::waitingForLight);
